...
OUTPUT_PATH = f"{CLS_IMAGE_FOLDER}/taboo_results_{DATA_DIR}_{sequence_str}_data_diversity.pdf"
...
def load_results(json_dir: str, required_verbalizer_prompt: str | None = None):
    ...
    # Apply filename filter (exclude files)
    if FILTER_FILENAMES:
        filtered_files = []
...
def plot_by_keyword_with_extras(
    ...
    plt.savefig(png_path, dpi=300, bbox_inches="tight")
    print(f"\nPlot saved as '{path}' and '{png_path}'")
    # plt.show()
...
def plot_per_word_accuracy(results_by_lora_word):
    ...
    for lora_path, word_accuracies in results_by_lora_word.items():
        ...
        print(f"Saved per-word plot: {filename} and {png_filename}")
        plt.close()
