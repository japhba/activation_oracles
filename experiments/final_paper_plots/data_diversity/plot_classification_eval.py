...
OUTPUT_PATH_BASE = f"{CLS_IMAGE_FOLDER}/classification_results_{DATA_DIR}_data_diversity"
...
def load_results_from_folder(folder_path, verbose=False):
    ...
    # Apply filename include filter (only include files that contain at least one string)
...
def _plot_split(
    ...
    for name in names:
        if name in CUSTOM_LABELS and CUSTOM_LABELS[name]:
            temp_labels.append(CUSTOM_LABELS[name])
        else:
            temp_labels.append(name)

    # Define ordering priority: Context Prediction (Data Matched) comes after highlighted, then LatentQA + Classification
    def get_sort_key(idx):
        if idx == m:
            return -1  # Highlighted bar stays first
        label = temp_labels[idx]
        if "(Data Matched)" in label:
    ...
    remaining_indices = [i for i in range(len(names)) if i != m]
    remaining_indices.sort(key=get_sort_key)
    order = [m] + remaining_indices
    ...
    print(f"\nPlot saved as '{output_path}' and '{png_path}'")
